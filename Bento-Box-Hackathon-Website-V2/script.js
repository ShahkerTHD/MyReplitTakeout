function createGlassSquare() {
  const square = document.createElement('div');
  square.classList.add('glass-square');
  ...
  square.style.animationDuration = `${duration}s, 3s`;

  document.getElementById('background-animation').appendChild(square);

  // Remove the square after it has fallen
  setTimeout(() => {
      square.remove();
  }, duration * 1000);
}
...
function startAnimation() {
  setInterval(createGlassSquare, 300); // Create a new square every 300ms
}
